...
export function generateQuestion(template: QuestionTemplate, seed: string | number): QuestionInstance {
  ...
  if (template.generator.kind === "nextNumber") {
    ...
    else if (skill === "1.L.2.b") { answer = "Where is my hat?"; choices = [answer, "Where is my hat.", "Where is my hat!"]; values = { question: "Which punctuation mark belongs at the end of this question?" }; explanation = `Questions end with a question mark.`; }
    ...
    else if (skill === "1.L.2.d") { const rows = [["ship", "sip", "shop"], ["rain", "ran", "run"]][random.integer(0, 1)]; answer = rows[0]; choices = rows; values = { question: "Which word has the spelling pattern named by the sound /sh/ or /ai/?" }; explanation = `${answer} uses the common spelling pattern.`; }
