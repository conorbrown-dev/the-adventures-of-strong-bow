...
export function generateQuestion(template: QuestionTemplate, seed: string | number): QuestionInstance {
  ...
  if (template.generator.kind === "nextNumber") {
    ...
    canonicalAnswer = null; interaction = adultScoredInteraction(skill, prompts[skill], "Common Core State Standards for English Language Arts"); values = { question: prompts[skill] }; explanation = "Your adult will decide whether this skill was demonstrated.";
    ...
    const skill = String(configuration.skill);
    const standard = oklahomaPhysicalEducationStandards.find((item) => item.officialId === skill);
    ...
    const items: Record<string, { question: string; answer: string; choices: string[]; explanation: string }> = {
      ...
      "K.RF.3.b": { question: "Which vowel letter spells the middle short sound in map?", answer: "a", choices: ["a", "i", "o"], explanation: "The letter a spells the short vowel sound in map." },
      "K.RF.3.c": { question: "Which common word should you know by sight?", answer: "the", choices: ["the", "xylophone", "pterodactyl"], explanation: "The is a common high-frequency word readers learn to recognize by sight." },
      "K.RF.3.d": { question: "Map and mop are spelled almost the same. Which part changes?", answer: "the middle sound, a to o", choices: ["the middle sound, a to o", "the first sound, m to s", "the last sound, p to t"], explanation: "Map has a in the middle, and mop has o in the middle." },
    ...
    const prompt = prompts[skill] ?? "Complete the activity with an adult.";
    canonicalAnswer = null; interaction = adultScoredInteraction(skill, prompt, "Common Core State Standards for English Language Arts"); values = { question: prompt }; explanation = "Your adult will decide whether this skill was demonstrated.";
