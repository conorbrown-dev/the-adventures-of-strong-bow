...
function parameters(template: CatalogTemplate): Record<string, unknown> {
  ...
  if (template.generatorKind === "oklahomaFineArtsAdult") return { skill: template.standardId };
  if (template.generatorKind === "oklahomaComputerScienceAdult") return { skill: template.standardId };
  if (template.generatorKind === "oklahomaInformationLiteracyAdult") return { skill: template.standardId };
  if (template.generatorKind === "oklahomaMathAdult") return { skill: template.standardId };
  if (template.generatorKind === "oklahomaElaAdult") return { skill: template.standardId };
...
function prompt(template: CatalogTemplate): { text: string; audioText: string; instructions: string } {
  ...
  if (template.generatorKind === "oklahomaFineArtsAdult") return { text: "{{question}}", audioText: "{{question}}", instructions: "Create, perform, respond, or reflect with an adult, then have the adult score what you demonstrated." };
  if (template.generatorKind === "oklahomaComputerScienceAdult") return { text: "{{question}}", audioText: "{{question}}", instructions: "Work with an adult using an unplugged activity or an appropriate device, then have the adult score what you demonstrated." };
  if (template.generatorKind === "oklahomaInformationLiteracyAdult") return { text: "{{question}}", audioText: "{{question}}", instructions: "Research safely with an adult, then have the adult score what you demonstrated." };
  if (template.generatorKind === "oklahomaMathAdult") return { text: "{{question}}", audioText: "{{question}}", instructions: "Use hands-on math tools with an adult, then have the adult score what you demonstrated." };
  if (template.generatorKind === "oklahomaElaAdult") return { text: "{{question}}", audioText: "{{question}}", instructions: "Read, talk, write, draw, or create with an adult, then have the adult score what you demonstrated." };
