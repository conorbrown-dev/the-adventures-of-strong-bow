...
- `ccssi.zip`: official machine-readable Common Core identifiers and metadata
...
The XML package is the import source of truth. The PDFs are human-review references.
Do not scrape unofficial standards websites.
...
1. reads the official `ccssi.zip`;
2. inspects the actual XML structure rather than assuming filenames;
3. imports only Mathematics and ELA standards for grades K, 1, 2, 3, 4, and 5;
4. preserves:
   - official identifier;
   ...
   - source URI or source package reference;
...
- official import filtering K–5 correctly;
- import idempotency;
