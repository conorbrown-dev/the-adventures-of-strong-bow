...
export class FossilDigTitleScene extends Phaser.Scene {
  ...
  private createButton(
    ...
    const button = this.add
      ...
      .setDepth(7);
    const source = button.texture.getSourceImage() as {
      width: number;
      height: number;
    };
  ...
  private refreshSelection(): void {
    this.buttons.forEach((entry, index) => {
      ...
      entry.hitZone.setSize(source.width * baseScale, source.height * baseScale);
    });
  }
  ...
  private startGame(): void {
    ...
    this.time.delayedCall(240, () => {
      this.scene.start(SCENE_KEYS.MAIN_MENU, { family: "fossil-dig" });
    });
  }
  ...
  private showSettingsOverlay(): void {
    ...
    const backdrop = this.add
      .rectangle(GAME_WIDTH / 2, GAME_HEIGHT / 2, GAME_WIDTH, GAME_HEIGHT, 0x1b120d, 0.74)
      .setDepth(10)
      .setInteractive();
    ...
    const title = this.add
      .text(GAME_WIDTH / 2, 176, "Fossil Dig Settings", {
    ...
    const doneButton = this.createPanelButton(GAME_WIDTH / 2, 614, "Done", () =>
      this.hideOverlay()
    );
