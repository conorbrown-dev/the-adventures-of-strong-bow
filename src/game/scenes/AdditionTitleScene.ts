...
import { type AdditionLayout, type AdditionSettings, loadAdditionSettings, saveAdditionSettings } from "../settings/additionSettings";
...
export class AdditionTitleScene extends Phaser.Scene {
  ...
  private showSettings(): void {
    ...
    items.push(this.add.rectangle(GAME_WIDTH / 2, GAME_HEIGHT / 2, 620, 500, 0x120b20, 0.98).setStrokeStyle(3, PURPLE));
    ...
    const done = this.add.rectangle(GAME_WIDTH / 2, 600, 170, 56, CYAN).setStrokeStyle(2, 0xffffff, 0.8);
    const doneText = this.add.text(GAME_WIDTH / 2, 600, "DONE", { fontFamily: "Arial Black, Trebuchet MS, sans-serif", fontSize: "20px", color: "#090610" }).setOrigin(0.5);
    const doneZone = this.add.zone(GAME_WIDTH / 2, 600, 170, 56).setInteractive({ useHandCursor: true }).on("pointerup", () => { saveAdditionSettings(this.settings); this.settingsPanel?.destroy(true); this.settingsPanel = undefined; });
