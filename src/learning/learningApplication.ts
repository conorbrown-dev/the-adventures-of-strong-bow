...
export const learningApplication = {
  async start(learnerId: string, purpose: LearningMode, proctorCode?: string, grade = "K", subject?: "ELA" | "MATH" | "SCIENCE"): Promise<SessionView> { const session = await studentApi<SessionView>("/curriculum/learning/sessions", "POST", { learnerId, purpose, grade, ...(subject ? { subject } : {}), ...(proctorCode ? { proctorCode } : {}) }); save(session); return session; },
  ...
  async updateSubjectLevel(studentId: string, subject: "ELA" | "MATH" | "SCIENCE", grade: "K" | "GRADE_1" | "GRADE_2", verificationCode: string): Promise<StudentPlacement> { return studentApi<StudentPlacement>(`/students/${studentId}/subject-level`, "PUT", { subject, grade, verificationCode }); },
