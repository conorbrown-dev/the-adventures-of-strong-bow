...
export type LearningMode = "practice" | "diagnostic" | "proctored";
...
export const learningApplication = {
  async start(learnerId: string, purpose: LearningMode, proctorCode?: string, grade = "K"): Promise<SessionView> { const session = await studentApi<SessionView>("/curriculum/learning/sessions", "POST", { learnerId, purpose, grade, ...(proctorCode ? { proctorCode } : {}) }); save(session); return session; },
