...
export function deriveQuestionSeed(sessionSeed: number, position: number): number { return Math.abs(Math.imul(sessionSeed ^ (position + 1), 2654435761)) >>> 0; }
...
export function nextQuestion(seed: number, subject: LearningSubject | "Mixed" = "Mixed", grade: LearningGrade = "K"): LearningQuestion {
  if (grade === "1") {
    if (subject === "ELA" || (subject === "Mixed" && seed % 2 === 0)) { const pair = pick([["cap", "cape", "cat"], ["kit", "kite", "kid"], ["hop", "hope", "hot"], ["tub", "tube", "tug"], ["pet", "Pete", "pen"]] as const, seed); return { id: `g1-decode-${seed}`, templateId: "1.rf.3.silent-e-decode", standardId: "1.RF.3", subject: "ELA", grade, skill: "Silent-e decoding", prompt: "Which word has a long vowel sound?", audioText: "Which word has a long vowel sound?", choices: [pair[0], pair[1], pair[2]], answer: pair[1], explanation: `${pair[1]} ends with silent e, so its vowel says its name.`, domain: "Reading foundational skills" }; }
  ...
  if (grade === "2") {
    ...
    if (seed % 2 === 0) { const hour = seed % 11 + 1; const minute = (Math.floor(seed / 11) % 12) * 5; const answer = `${hour}:${String(minute).padStart(2, "0")}`; return { id: `g2-clock-${seed}`, templateId: "2.md.c.7.clock-time", standardId: "2.MD.C.7", subject: "Math", grade, skill: "Read a clock", prompt: "What time does this clock show?", audioText: "What time does this clock show?", choices: [answer, `${hour}:${String((minute + 5) % 60).padStart(2, "0")}`, `${hour}:${String((minute + 30) % 60).padStart(2, "0")}`], answer, explanation: `The clock shows ${answer}.`, domain: "Measurement and data", visual: `🕒 ${answer}` }; }
    const money = pick([["25¢ + 5¢", "30¢"], ["10¢ + 10¢ + 5¢", "25¢"], ["25¢ + 10¢ + 10¢", "45¢"], ["5¢ + 5¢ + 5¢", "15¢"]] as const, seed); return { id: `g2-money-${seed}`, templateId: "2.md.c.8.money-total", standardId: "2.MD.C.8", subject: "Math", grade, skill: "Count coins", prompt: "How much money is shown?", audioText: "How much money is shown?", choices: [money[1], "20¢", "35¢"], answer: money[1], explanation: `${money[0]} equals ${money[1]}.`, domain: "Measurement and data", visual: money[0] };
  }
  const isEla = subject === "Mixed" ? seed % 2 === 0 : subject === "ELA";
  if (isEla) {
    const letters = [["B", "b"], ["M", "m"], ["T", "t"], ["F", "f"], ["R", "r"], ["S", "s"], ["D", "d"], ["P", "p"]] as const; const pair = pick(letters, Math.floor(seed / 3)); const rhymes = [["cat", "hat", "sun"], ["pig", "wig", "map"], ["bug", "rug", "pen"], ["hop", "mop", "tree"], ["fan", "man", "log"]] as const;
    ...
    const words = pick(rhymes, Math.floor(seed / 3)); return { id: `rhyme-${seed}`, templateId: "k.rf.2.a.rhyme", standardId: "K.RF.2.a", subject: "ELA", grade, skill: "Rhyming words", prompt: `Which word rhymes with ${words[0]}?`, audioText: `Which word rhymes with ${words[0]}?`, choices: [words[1], words[2], "dog"], answer: words[1], explanation: `${words[0]} rhymes with ${words[1]} because both words end with the same sound.`, domain: "Reading foundational skills" };
  }
