    ...
    <details className="adult-tools group rounded-3xl p-5 sm:p-7">
      ...
      <div className="adult-tools-content mt-6 pt-6">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          {selectedSubject !== "SCIENCE" && selectedSubject !== "SOCIAL_STUDIES" && selectedSubject !== "HEALTH" && selectedSubject !== "PHYSICAL_EDUCATION" && selectedSubject !== "FINE_ARTS" && selectedSubject !== "COMPUTER_SCIENCE" && selectedSubject !== "INFORMATION_LITERACY" && <button className="secondary !rounded-2xl" disabled={isLoading || !proctorCode} onClick={() => void start("placement")}>NEW-STUDENT PLACEMENT</button>}
          {selectedSubject !== "SCIENCE" && selectedSubject !== "SOCIAL_STUDIES" && selectedSubject !== "HEALTH" && selectedSubject !== "PHYSICAL_EDUCATION" && selectedSubject !== "FINE_ARTS" && selectedSubject !== "COMPUTER_SCIENCE" && selectedSubject !== "INFORMATION_LITERACY" && <button className="secondary !rounded-2xl" disabled={isLoading || !proctorCode} onClick={() => void start("proctored")}>PROCTORED CHECK</button>}
          {(selectedSubject === "ELA" || selectedSubject === "SCIENCE" || selectedSubject === "SOCIAL_STUDIES" || selectedSubject === "HEALTH" || selectedSubject === "PHYSICAL_EDUCATION" || selectedSubject === "FINE_ARTS" || selectedSubject === "COMPUTER_SCIENCE" || selectedSubject === "INFORMATION_LITERACY") && <button className="secondary !rounded-2xl" disabled={isLoading || !proctorCode} onClick={() => void start("adultScored")}>{selectedSubject === "SCIENCE" ? "SCIENCE INVESTIGATION" : selectedSubject === "SOCIAL_STUDIES" ? "SOCIAL STUDIES INQUIRY" : selectedSubject === "HEALTH" ? "HEALTH ACTIVITY" : selectedSubject === "PHYSICAL_EDUCATION" ? "MOVEMENT ACTIVITY" : selectedSubject === "FINE_ARTS" ? "FINE ARTS ACTIVITY" : selectedSubject === "COMPUTER_SCIENCE" ? "COMPUTER SCIENCE ACTIVITY" : selectedSubject === "INFORMATION_LITERACY" ? "INFORMATION LITERACY INQUIRY" : "ADULT-SCORED ELA"}</button>}
        </div>
...
export function LearningApp(): JSX.Element {
  ...
  return <main className="learning-app"><section className="learning-shell">
    ...
    {isQuestion && session && <section className="learning-question">
      <p className="eyebrow">{location.pathname === "/learning/adult-scored" ? selectedSubject === "SCIENCE" ? "SCIENCE INVESTIGATION" : selectedSubject === "SOCIAL_STUDIES" ? "SOCIAL STUDIES INQUIRY" : selectedSubject === "HEALTH" ? "HEALTH ACTIVITY" : selectedSubject === "PHYSICAL_EDUCATION" ? "MOVEMENT ACTIVITY" : selectedSubject === "FINE_ARTS" ? "FINE ARTS ACTIVITY" : selectedSubject === "COMPUTER_SCIENCE" ? "COMPUTER SCIENCE ACTIVITY" : selectedSubject === "INFORMATION_LITERACY" ? "INFORMATION LITERACY INQUIRY" : "ADULT-SCORED ELA CHECK" : `QUESTION ${session.position + 1} OF ${session.length}`}</p>
