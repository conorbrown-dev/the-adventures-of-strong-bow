...
type LearningSubject = "ELA" | "MATH" | "SCIENCE" | "SOCIAL_STUDIES" | "HEALTH" | "PHYSICAL_EDUCATION" | "FINE_ARTS" | "COMPUTER_SCIENCE";
  ...
  return <section className="learning-dashboard mx-auto max-w-4xl">
    ...
    <section aria-labelledby="subject-heading" className="learning-card subject-card mb-7 rounded-3xl p-5 sm:p-7">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {(["ELA", "MATH", "SCIENCE", "SOCIAL_STUDIES", "HEALTH", "PHYSICAL_EDUCATION", "FINE_ARTS", "COMPUTER_SCIENCE"] as const).map((subject) => {
          ...
          return <button className={`${isSelected ? "selected" : "secondary"} subject-choice !min-h-32 !rounded-2xl !p-6 !text-left`} key={subject} onClick={() => setSelectedSubject(subject)} aria-pressed={isSelected}>
            <span aria-hidden="true" className="subject-icon">{subject === "ELA" ? "Aa" : subject === "MATH" ? "1+2" : subject === "SCIENCE" ? "✦" : subject === "SOCIAL_STUDIES" ? "⌂" : subject === "HEALTH" ? "♥" : subject === "PHYSICAL_EDUCATION" ? "↟" : subject === "FINE_ARTS" ? "✎" : "⌘"}</span>
            <span><strong className="block text-xl">{subject === "ELA" ? "Reading & Language" : subject === "MATH" ? "Math" : subject === "SCIENCE" ? "Science" : subject === "SOCIAL_STUDIES" ? "Social Studies" : subject === "HEALTH" ? "Health" : subject === "PHYSICAL_EDUCATION" ? "Physical Education" : subject === "FINE_ARTS" ? "Fine Arts" : "Computer Science"}</strong><small className="mt-1 block text-sm">{gradeName(student?.curriculumLevels?.[subject] ?? student?.grade)}</small></span>
          </button>;
        })}
      </div>
    </section>
    ...
    <details className="adult-tools group rounded-3xl p-5 sm:p-7">
      ...
      <div className="adult-tools-content mt-6 pt-6">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          {selectedSubject !== "SCIENCE" && selectedSubject !== "SOCIAL_STUDIES" && selectedSubject !== "HEALTH" && selectedSubject !== "PHYSICAL_EDUCATION" && selectedSubject !== "FINE_ARTS" && selectedSubject !== "COMPUTER_SCIENCE" && <button className="secondary !rounded-2xl" disabled={isLoading || !proctorCode} onClick={() => void start("placement")}>NEW-STUDENT PLACEMENT</button>}
          {selectedSubject !== "SCIENCE" && selectedSubject !== "SOCIAL_STUDIES" && selectedSubject !== "HEALTH" && selectedSubject !== "PHYSICAL_EDUCATION" && selectedSubject !== "FINE_ARTS" && selectedSubject !== "COMPUTER_SCIENCE" && <button className="secondary !rounded-2xl" disabled={isLoading || !proctorCode} onClick={() => void start("proctored")}>PROCTORED CHECK</button>}
          {(selectedSubject === "ELA" || selectedSubject === "SCIENCE" || selectedSubject === "SOCIAL_STUDIES" || selectedSubject === "HEALTH" || selectedSubject === "PHYSICAL_EDUCATION" || selectedSubject === "FINE_ARTS" || selectedSubject === "COMPUTER_SCIENCE") && <button className="secondary !rounded-2xl" disabled={isLoading || !proctorCode} onClick={() => void start("adultScored")}>{selectedSubject === "SCIENCE" ? "SCIENCE INVESTIGATION" : selectedSubject === "SOCIAL_STUDIES" ? "SOCIAL STUDIES INQUIRY" : selectedSubject === "HEALTH" ? "HEALTH ACTIVITY" : selectedSubject === "PHYSICAL_EDUCATION" ? "MOVEMENT ACTIVITY" : selectedSubject === "FINE_ARTS" ? "FINE ARTS ACTIVITY" : selectedSubject === "COMPUTER_SCIENCE" ? "COMPUTER SCIENCE ACTIVITY" : "ADULT-SCORED ELA"}</button>}
        </div>
        <div className="level-panel mt-7 rounded-2xl p-5">
          <h3 className="mb-3 text-lg font-black">Set subject level</h3>
          <div className="flex flex-wrap items-end gap-3">
            <label className="!my-0">Level for {selectedSubject === "ELA" ? "Reading & Language" : selectedSubject === "MATH" ? "Math" : selectedSubject === "SCIENCE" ? "Science" : selectedSubject === "SOCIAL_STUDIES" ? "Social Studies" : selectedSubject === "HEALTH" ? "Health" : selectedSubject === "PHYSICAL_EDUCATION" ? "Physical Education" : selectedSubject === "FINE_ARTS" ? "Fine Arts" : "Computer Science"}<select value={placementGrade} onChange={(event) => setPlacementGrade(event.target.value as LearningLevel)}><option value="K">Kindergarten</option><option value="GRADE_1">Grade 1</option><option value="GRADE_2">Grade 2</option></select></label>
...
export function LearningApp(): JSX.Element {
  ...
  return <main className="learning-app"><section className="learning-shell">
    ...
    {isQuestion && session && <section className="learning-question">
      <p className="eyebrow">{location.pathname === "/learning/adult-scored" ? selectedSubject === "SCIENCE" ? "SCIENCE INVESTIGATION" : selectedSubject === "SOCIAL_STUDIES" ? "SOCIAL STUDIES INQUIRY" : selectedSubject === "HEALTH" ? "HEALTH ACTIVITY" : selectedSubject === "PHYSICAL_EDUCATION" ? "MOVEMENT ACTIVITY" : selectedSubject === "FINE_ARTS" ? "FINE ARTS ACTIVITY" : selectedSubject === "COMPUTER_SCIENCE" ? "COMPUTER SCIENCE ACTIVITY" : "ADULT-SCORED ELA CHECK" : `QUESTION ${session.position + 1} OF ${session.length}`}</p>
